...
test "int declaration with initializer" {
    ...
    try expectEqual(init.expr.literal.number, 60);
}
test "string declaration with initializer" {
    ...
    try expectEqual(init.expr.interpolated_string.len, 1);
    try expectEqualStrings(init.expr.interpolated_string[0].text, "test");
}
test "bool declaration with initializer" {
    ...
    const init = stmt.var_decl.init.?;
    try expect(tag(init.expr.*) == .literal);
    try expectEqual(init.expr.literal.boolean, true);
}
...
test "variable expression" {
    ...
    try expectEqualStrings(outer.left.binary.left.variable, "x");
    try expectEqual(.variable, std.meta.activeTag(outer.left.binary.right.*));
    try expectEqualStrings(outer.left.binary.right.variable, "y");
    try expectEqual(.variable, std.meta.activeTag(outer.right.*));
    try expectEqualStrings(outer.right.variable, "z");
}
test "parentheses" {
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    ...
    const init = stmt.var_decl.init.?;
    try expect(tag(init.expr.*) == .literal);
    try expectEqual(init.expr.literal.number, 42);
}
test "addition" {
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
...
test "parse operator precedence" {
    ...
    try expectEqual(false, neq.right.literal.boolean);
    ...
    try expectEqual(@as(i64, 9), eq.right.literal.number);
    ...
    try expectEqual(@as(i64, 8), ge.right.literal.number);
...
test "array declaration" {
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    const tree = try lexnparse(arena.allocator(), "int arr[10];");
    const stmt = tree.program[0];
...
test "array literal initializer" {
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    const tree = try lexnparse(arena.allocator(), "int arr[3] = {1,2,3};");
    const stmt = tree.program[0];
    const init = stmt.var_decl.init.?;
...
test "while statement" {
    ...
    try expectEqual(true, stmt.while_stmt.condition.literal.boolean);
...
test "interpolated string with multiple expressions" {
    ...
    try expectEqual(@as(usize, 3), init.expr.interpolated_string.len);
}
...
test "array literal with expressions" {
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    const tree = try lexnparse(arena.allocator(), "int arr[3] = {1+2,3*4,test()};");
    const stmt = tree.program[0];
    const init = stmt.var_decl.init.?;
    try expectEqual(.array_literal, tag(init));
    try expectEqual(@as(usize, 3), init.array_literal.len);
